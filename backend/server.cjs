...
const corsOptions = {
  origin: 'https://satellite-analysis.vercel.app',
  methods: ['GET', 'POST'],
...
app.post('/api/generate-video', upload.fields([{ name: 'image1' }, { name: 'image2' }]), (req, res) => {
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    res.json({ videoUrl: `https://satellite-analysis-m4gg.vercel.app/uploads/${path.basename(outputVideoPath)}` });
  });
});

app.listen(port, () => {
  console.log(`Server running on https://satellite-analysis-m4gg.vercel.app/`);
});
